...
def check_unitapi_installed():
    """Check if unitapi package is installed."""
    try:
        import unitapi
        return True
    except ImportError:
...
def install_unitapi():
    """Install unitapi package in development mode."""
    try:
        subprocess.run(
        ...
        print(f"Failed to install unitapi: {e.stderr}")
        return False
...
def main():
    ...
    if not check_unitapi_installed():
        print("unitapi package not found. Installing in development mode...")
