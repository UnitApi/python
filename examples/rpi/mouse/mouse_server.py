#!/usr/bin/env python3
"""
Raspberry Pi Mouse Server Example

This script runs on a Raspberry Pi to register a mouse device with the UnitAPI server.
It allows remote clients to control the mouse on the Raspberry Pi.
"""
...
class RPiMouseServer:
    def __init__(
        ...
        """
        Initialize Raspberry Pi Mouse server.
    ...
    async def setup_mouse_device(self, device_id: str = "mouse_rpi", name: str = "Raspberry Pi Mouse"):
        """
        ...
        try:
            ...
            self.mouse_device = MouseDevice(
                device_id=device_id,
                name=name,
                metadata={"type": "optical", "device_type": "raspberry_pi"}
            )
    ...
    async def start(self):
        ...
        try:
            ...
            self.logger.info(f"Raspberry Pi Mouse Server started on {self.server.host}:{self.server.port}")
...
async def main():
    """
    Run the Raspberry Pi Mouse server.
    """
    parser = argparse.ArgumentParser(description='UnitAPI Raspberry Pi Mouse Server')
    parser.add_argument('--debug', action='store_true', help='Enable debug logging')
    parser.add_argument('--host', default='0.0.0.0', help='Server host address')
    parser.add_argument('--port', type=int, default=7890, help='Server port')
    parser.add_argument('--device-id', default='mouse_rpi', help='Mouse device ID')
    parser.add_argument('--name', default='Raspberry Pi Mouse', help='Mouse device name')
    
    args = parser.parse_args()
    
    server = RPiMouseServer(
        host=args.host,
        port=args.port,
        debug=args.debug
    )
