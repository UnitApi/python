#!/usr/bin/env python3
"""
PC Microphone Server Example

This script demonstrates how to create a UnitAPI server that exposes microphone functionality
on a PC. This server can be used with the microphone_client.py client example.
"""
...
class PCMicrophoneServer:
    def __init__(
        ...
        """
        Initialize PC Microphone server.
    ...
    async def start_recording(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            output_file = params.get('output_file', f'pc_audio_{int(time.time())}.{audio_format}')
    ...
    async def _start_microphone_recording(
        ...
        try:
            ...
            # For other formats, use ffmpeg if available
            else:
                import subprocess
                ...
                if has_ffmpeg:
                    ...
                    if audio_format.lower() == 'mp3':
                        cmd = [
                            ...
                            '-i', 'audio=Microphone' if sys.platform == 'win32' else 'default',
                            '-acodec', 'libmp3lame',
                            '-ab', '192k',
                            '-ac', str(channels),
                            '-ar', str(sample_rate),
                    ...
                    # Fall back to PyAudio and convert later
                    self.logger.warning(f"ffmpeg not available, recording in WAV format")
                    
                    # Record in WAV format
                    wav_file = output_file.rsplit('.', 1)[0] + '.wav'
                    recording = await self._start_microphone_recording(
                        sample_rate, channels, 'wav', wav_file
                    )
                    
                    # Add conversion info
                    recording['convert_to'] = audio_format
                    recording['final_file'] = output_file
    ...
    async def _stop_microphone_recording(self, recording: Dict[str, Any]) -> None:
        ...
        try:
            if 'thread' in recording and 'stop_event' in recording:
                ...
                if 'convert_to' in recording and 'final_file' in recording:
                    await self._convert_audio_format(
                        recording['wavefile'].name,
                        recording['final_file'],
                        recording['convert_to']
                    )
                        
            elif 'process' in recording:
                # Stop ffmpeg process
                try:
    ...
    async def _convert_audio_format(
        ...
        try:
            ...
            if has_ffmpeg:
                ...
                self.logger.info(f"Converted audio file to {output_format} format")
            else:
                self.logger.warning(f"ffmpeg not available, cannot convert to {output_format} format")
    ...
    async def start(self):
        ...
        try:
            ...
            self.logger.info(f"PC Microphone Server started on {self.server.host}:{self.server.port}")
...
async def main():
    """
    Run the PC Microphone server.
    """
    parser = argparse.ArgumentParser(description='UnitAPI PC Microphone Server')
