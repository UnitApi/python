#!/usr/bin/env python3
"""
PC Camera Server Example

This script demonstrates how to create a UnitAPI server that exposes a PC webcam
functionality. This server can be used with the camera_client.py client example.
"""
...
class PCCameraServer:
    ...
    async def setup_camera_device(self):
        ...
        try:
            ...
            if not camera_available:
                self.logger.warning("Webcam not detected. Using virtual camera.")
    ...
    async def _capture_from_camera(
        ...
        """
        Capture an image from the physical webcam.
    ...
    def _generate_test_image(
        ...
        try:
            ...
            cv2.putText(img, 'PC Webcam', (width//4, height//2), 
    ...
    async def start_video_recording(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        Start video recording from the webcam.
        ...
        try:
            ...
            video_format = params.get('format', 'mp4')
            output_file = params.get('output_file', f'webcam_video_{int(time.time())}.mp4')
    ...
    async def _start_camera_recording(
        ...
        """
        Start actual video recording from the physical webcam.
        ...
        try:
            ...
            # Determine fourcc code based on format
    ...
    async def start(self):
        ...
        try:
            ...
            self.logger.info(f"PC Camera Server started on {self.server.host}:{self.server.port}")
...
async def main():
    """
    Run the PC Camera server.
    """
    parser = argparse.ArgumentParser(description='UnitAPI PC Camera Server')
