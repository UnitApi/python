...
async def main():
    ...
    parser.add_argument('--config', default='speaker_config.star', 
                        help='Path to the Starlark configuration file (default: speaker_config.star)')
            ...
            for pipeline in config.get('pipelines', []):
                ...
                if hasattr(pipeline, 'target'):
                    if isinstance(pipeline.target, list):
                        logger.info(f"    Targets: {', '.join(pipeline.target)}")
                    else:
